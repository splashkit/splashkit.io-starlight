...
namespace BitmapCollisionsDemo
{
    public class Program
    {
        public static void Main()
        {
            ...
            if (BitmapCollision(skBmp, skBmpLoc.X, skBmpLoc.Y, fileBmp, fileBmpLoc.X, fileBmpLoc.Y))
            {
                WriteLine("SplashKit got a new file!");
            }

            if (BitmapCollision(skBmp, skBmpLoc.X, skBmpLoc.Y, bugBmp, bugBmpLoc.X, bugBmpLoc.Y))
            {
