---
title: Install SplashKit
description: Install SplashKit on Windows (MSYS2).
sidebar:
  label: 2. SplashKit
#   attrs:
#     class: windows
---

import { Steps } from '@astrojs/starlight/components';

[SplashKit](https://splashkit.io) is a beginner-friendly toolkit that simplifies creating engaging applications, including Graphical User Interfaces (GUIs), interactive games, and media-rich projects. It offers a variety of pre-built functions and supports multiple programming languages, such as C++, C#, and Python, making it accessible to both new and experienced developers.

Follow these steps to set up SplashKit on Windows with MSYS2 and start building interactive, visually appealing applications!

## Installation Steps

<Steps>

1. **Run the SplashKit Installer**

   In your MSYS2 Terminal, paste and run the following command to download and install SplashKit:

    ```shell
    bash <(curl -s https://raw.githubusercontent.com/splashkit/skm/master/install-scripts/skm-install.sh)
    ```

    This command downloads the installer directly from the [SplashKit homepage](https://splashkit.io).

    {/* TODO: Add gif */}

    ---

2. **Verify the Installation**
   Restart your MSYS2 terminal, then enter the following command to check if SplashKit Manager (`skm`) is installed:

    ```shell
    skm
    ```

    You should see messages confirming the installation, such as:

    ```shell
    SplashKit is installed successfully!
    Missing skm command. For help use 'skm help'
    ```

    *Tip:* Use `skm help` to explore available commands for working with SplashKit.

    ---

3. **Install SplashKit Globally**
   For easier access, install SplashKit globally by running:

    ```shell
    skm global install
    ```

</Steps>
