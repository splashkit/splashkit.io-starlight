...
<Tabs syncKey="code-language">
  ...
  ```csharp
  var newGameData = Json.Create();
  newGameData.SetString("gameTitle", "My New Game");
  newGameData.SetBool("fullScreenMode", false);
  newGameData.SetNumber("numPlayers", 1);
  ```
...
<Tabs syncKey="code-language">
  ...
  newGameData.SetArray("levels", levelsArray);
  ```
...
<Tabs syncKey="code-language">
  ...
  ```csharp
  var screenSizeData = Json.Create();

  screenSizeData.SetNumber("width", 800);
  screenSizeData.SetNumber("height", 600);

  newGameData.SetObject("screenSize", screenSizeData);
  ```
...
<Tabs syncKey="code-language">
  ...
  ```csharp
  newGameData.ToFile("new_game_data.json");
...
<Tabs syncKey="code-language">
  ...
  var levelsArray = new List<string> { "level1", "level2", "level3" };
  newGameData.SetArray("levels", levelsArray);
...
<Tabs syncKey="code-language">
  ...
  ```csharp
  var playerData = Json.Create();
  playerData.SetString("name", "Hero");

  var statsData = Json.Create();
  statsData.SetNumber("health", 100);
  statsData.SetNumber("mana", 50);
  statsData.SetNumber("strength", 75);

  playerData.SetObject("stats", statsData);
  ```
...
<Tabs syncKey="code-language">
  ...
  ```csharp
  var existingData = Json.FromFile("new_game_data.json");
  existingData.SetObject("character", playerData);

  existingData.ToFile("modified_game_data.json");
  ```
...
<Tabs syncKey="code-language">
  ...
  ```csharp
  // Load our JSON
  var modifiedGameData = Json.FromFile("modified_game_data.json");
  // Retrieve Character JSON object from the file.
  var character = modifiedGameData.ReadObject("character");
  // Retrieve the Stats JSON object from the Character JSON
  var stats = character.ReadObject("stats");
  // Retrieve the value of health from the stats JSON object
  int hp = stats.ReadNumberAsInt("health");
  ```
