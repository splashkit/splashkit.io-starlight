...
RaspiSetMode(increaseBtnPin, GpioPinMode.GpioInput);
RaspiSetMode(decreaseBtnPin, GpioPinMode.GpioInput);
...
while(!AnyKeyPressed())
{
    ...
    {
        if(RaspiRead(increaseBtnPin) == GpioPinValue.GpioHigh)
        {
            brightness += 25;
        ...
        if(RaspiRead(decreaseBtnPin) == GpioPinValue.GpioHigh)
        {
            brightness -= 25;
            if(brightness < 0)
                brightness = 0;
...
namespace RaspberryPiPWMButtonControl
{
    public class Program
    {
        public static void Main()
        {
            ...
            SplashKit.RaspiSetMode(increaseBtnPin, GpioPinMode.GpioInput);
            SplashKit.RaspiSetMode(decreaseBtnPin, GpioPinMode.GpioInput);
            ...
            while(!SplashKit.AnyKeyPressed())
            {
                ...
                if(currentTime - lastReadTime > readInterval)
                {
                    if(SplashKit.RaspiRead(increaseBtnPin) == GpioPinValue.GpioHigh)
                    {
                        brightness += 25;
                        if(brightness > maxBrightness)
                            brightness = maxBrightness;
                    ...
                    if(SplashKit.RaspiRead(decreaseBtnPin) == GpioPinValue.GpioHigh)
                    {
                        brightness -= 25;
                        if(brightness < 0)
                            brightness = 0;
...
int main()
{
    ...
    connection pi = remote_raspi_init("pwm_control_pi", "192.168.1.100", 8888);
    gpio_pin led_pin = PIN_11;
    gpio_pin increase_btn_pin = PIN_13;
    gpio_pin decrease_btn_pin = PIN_29;

    remote_raspi_set_mode(pi, led_pin, GPIO_OUTPUT);
    remote_raspi_set_pwm_dutycycle(pi, led_pin, brightness);

    remote_raspi_set_mode(pi, increase_btn_pin, GPIO_INPUT);
    remote_raspi_set_mode(pi, decrease_btn_pin, GPIO_INPUT);

    remote_raspi_set_pull_up_down(pi, increase_btn_pin, PUD_DOWN);
    remote_raspi_set_pull_up_down(pi, decrease_btn_pin, PUD_DOWN);
    ...
    while(!any_key_pressed())
    {
        ...
        if(current_time - last_read_time > read_interval)
        {
            if(remote_raspi_read(pi, increase_btn_pin) == GPIO_HIGH)
            {
                brightness += 25;
                if(brightness > max_brightness)
                    brightness = max_brightness;
            ...
            if(remote_raspi_read(pi, decrease_btn_pin) == GPIO_HIGH)
            {
                brightness -= 25;
                if(brightness < 0)
                    brightness = 0;
...
// Connect to remote Raspberry Pi
Connection pi = RemoteRaspiInit("pwm_control_pi", "192.168.1.100", 8888);
GpioPin ledPin = GpioPin.Pin11;
GpioPin increaseBtnPin = GpioPin.Pin13;
GpioPin decreaseBtnPin = GpioPin.Pin29;

RemoteRaspiSetMode(pi, ledPin, GpioPinMode.GpioOutput);
RemoteRaspiSetPwmDutycycle(pi, ledPin, brightness);

RemoteRaspiSetMode(pi, increaseBtnPin, GpioPinMode.GpioInput);
RemoteRaspiSetMode(pi, decreaseBtnPin, GpioPinMode.GpioInput);

RemoteRaspiSetPullUpDown(pi, increaseBtnPin, PullUpDown.PudDown);
RemoteRaspiSetPullUpDown(pi, decreaseBtnPin, PullUpDown.PudDown);
...
while(!AnyKeyPressed())
{
    ...
    if(currentTime - lastReadTime > readInterval)
    {
        if(RemoteRaspiRead(pi, increaseBtnPin) == GpioPinValue.GpioHigh)
        {
            brightness += 25;
            if(brightness > maxBrightness)
                brightness = maxBrightness;
        ...
        if(RemoteRaspiRead(pi, decreaseBtnPin) == GpioPinValue.GpioHigh)
        {
            brightness -= 25;
            if(brightness < 0)
                brightness = 0;
...
namespace RaspberryPiPWMButtonControl
{
    public class Program
    {
        public static void Main()
        {
            ...
            // Connect to remote Raspberry Pi
            Connection pi = SplashKit.RemoteRaspiInit("pwm_control_pi", "192.168.1.100", 8888);
            GpioPin ledPin = GpioPin.Pin11;
            GpioPin increaseBtnPin = GpioPin.Pin13;
            GpioPin decreaseBtnPin = GpioPin.Pin29;

            SplashKit.RemoteRaspiSetMode(pi, ledPin, GpioPinMode.GpioOutput);
            SplashKit.RemoteRaspiSetPwmDutycycle(pi, ledPin, brightness);

            SplashKit.RemoteRaspiSetMode(pi, increaseBtnPin, GpioPinMode.GpioInput);
            SplashKit.RemoteRaspiSetMode(pi, decreaseBtnPin, GpioPinMode.GpioInput);

            SplashKit.RemoteRaspiSetPullUpDown(pi, increaseBtnPin, PullUpDown.PudDown);
            SplashKit.RemoteRaspiSetPullUpDown(pi, decreaseBtnPin, PullUpDown.PudDown);
            ...
            while(!SplashKit.AnyKeyPressed())
            {
                ...
                if(currentTime - lastReadTime > readInterval)
                {
                    if(SplashKit.RemoteRaspiRead(pi, increaseBtnPin) == GpioPinValue.GpioHigh)
                    {
                        brightness += 25;
                        if(brightness > maxBrightness)
                            brightness = maxBrightness;
                    ...
                    if(SplashKit.RemoteRaspiRead(pi, decreaseBtnPin) == GpioPinValue.GpioHigh)
                    {
                        brightness -= 25;
                        if(brightness < 0)
                            brightness = 0;
