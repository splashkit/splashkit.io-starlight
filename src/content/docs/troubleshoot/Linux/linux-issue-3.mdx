...
## `skm global install` Fails During Installation on Arch-Based Distributions

**Description:** This guide addresses the issue where the `skm global install` command fails during the installation process on Arch-based Linux distributions like Manjaro due to a dependency error with `libjxl` required by `SDL2_image`.
