...
### Bash Curl Command Does Nothing on MacOS

**Description:** This guide addresses the issue on MacOS where running the command `bash <(curl -s https://raw.githubusercontent.com/splashkit/skm/master/install-scripts/skm-install.sh)` does not trigger the expected installation. Follow the solutions below to resolve this.
...
2. **Solution 2: Temporarily Disable Firewall**
...
3. **Solution 3: Manually Download and Run the Script**
